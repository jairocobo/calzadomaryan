...
class Setting extends CI_Controller
{
    ...
    public function deleteSupplier()
    {
        ...
        if ($this->Constant_model->deleteData('suppliers', $supplier_id)) {
            $this->session->set_flashdata('alert_msg', array('success', 'Eliminar Proveedor', "Proveedor eliminado con éxito : $supplier_name."));
    ...
    public function deleteOutlet()
    {
        ...
        if ($this->Constant_model->deleteData('outlets', $outlet_id)) {
            $this->session->set_flashdata('alert_msg', array('success', 'Eliminar', "Eliminado con éxito : $outlet_name."));
    ...
    public function deleteUser()
    {
        ...
        if ($this->Constant_model->deleteData('users', $us_id)) {
            $this->session->set_flashdata('alert_msg', array('success', 'Borrar usuario', "Usuario eliminado con éxito : $us_name."));
    ...
    public function deletePaymentMethod()
    {
        ...
        if ($this->Constant_model->deleteData('payment_method', $pay_id)) {
            $this->session->set_flashdata('alert_msg', array('success', 'Eliminar método de pago', "Método de pago eliminado con éxito : $pay_name."));
    ...
    public function updateSupplier()
    {
        ...
        if (empty($name)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar proveedor', 'Por favor ingrese el nombre del proveedor!'));
            redirect(base_url().'setting/editsupplier?id='.$id);
        } elseif (empty($tel)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar proveedor', 'Por favor ingrese el número de teléfono del proveedor!'));
            redirect(base_url().'setting/editsupplier?id='.$id);
        } elseif (empty($address)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar proveedor', 'Por favor ingrese la dirección del proveedor!'));
            redirect(base_url().'setting/editsupplier?id='.$id);
        } else {
            $upd_data = array(
                'name' => $name,
            ...
            if ($this->Constant_model->updateData('suppliers', $upd_data, $id)) {
                $this->session->set_flashdata('alert_msg', array('success', 'Actualizar proveedor', "Proveedor actualizado con éxito : $name"));
                redirect(base_url().'setting/editsupplier?id='.$id);
            }
        }
    }
    ...
    public function insertSupplier()
    {
        ...
        if (empty($name)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir Proveedor', 'Por favor ingrese el nombre del proveedor!', "$name", "$email", "$tel", "$fax", "$address", "$tax"));
            redirect(base_url().'setting/addsupplier');
        } elseif (empty($tel)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir Proveedor', 'Por favor ingrese el número de teléfono del proveedor!', "$name", "$email", "$tel", "$fax", "$address", "$tax"));
            redirect(base_url().'setting/addsupplier');
        } elseif (empty($address)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir Proveedor', 'Por favor ingrese la dirección del proveedor!', "$name", "$email", "$tel", "$fax", "$address", "$tax"));
            redirect(base_url().'setting/addsupplier');
        } else {
            $ins_data = array(
                    'name' => $name,
            ...
            if ($this->Constant_model->insertData('suppliers', $ins_data)) {
                $this->session->set_flashdata('alert_msg', array('success', 'Añadir Proveedor', "Agregado exitosamente nuevo proveedor : $name", '', '', '', '', '', ''));
                redirect(base_url().'setting/addsupplier');
            }
        }
    }
    ...
    public function updatePaymentMethod()
    {
        ...
        if (empty($name)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar el método de pago', 'Por favor ingrese el método de pago!'));
            ...
            if ($this->Constant_model->updateData('payment_method', $upd_data, $id)) {
                $this->session->set_flashdata('alert_msg', array('success', 'Actualizar el método de pago', 'Método de pago actualizado con éxito.'));
                redirect(base_url().'setting/editpaymentmethod?id='.$id);
            }
        }
    }
    ...
    public function insertPaymentMethod()
    {
        ...
        if (empty($name)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo método de pago', 'Por favor ingrese el Nombre del Método de Pago!'));
            ...
            if ($this->Constant_model->insertData('payment_method', $ins_data)) {
                $this->session->set_flashdata('alert_msg', array('success', 'Añadir nuevo método de pago', 'Se agregó exitosamente un nuevo método de pago.'));
                redirect(base_url().'setting/addpaymentmethod');
            }
        }
    }
    ...
    public function updatePassword()
    {
        ...
        if (empty($pass)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar contraseña', 'Por favor ingrese su nueva contraseña!'));
            redirect(base_url().'setting/changePassword?id='.$id);
        } elseif (empty($conpass)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar contraseña', 'Por favor ingrese su contraseña de confirmación!'));
            redirect(base_url().'setting/changePassword?id='.$id);
        } elseif ($pass != $conpass) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar contraseña', 'Las contraseñas deben ser iguales!'));
            redirect(base_url().'setting/changePassword?id='.$id);
        } else {
            ...
            if ($this->Constant_model->updateData('users', $upd_data, $id)) {
                $this->session->set_flashdata('alert_msg', array('success', 'Actualizar contraseña', 'Se actualizó con éxito su nueva contraseña.'));
                redirect(base_url().'setting/changePassword?id='.$id);
            }
        }
    }
    ...
    public function updateUser()
    {
        ...
        if (empty($fn)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar usuario', 'Por favor ingrese el nombre completo!'));
            redirect(base_url().'setting/edituser?id='.$id);
        } elseif (empty($email)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar usuario', 'Por favor, introduzca la dirección de correo electrónico!'));
            redirect(base_url().'setting/edituser?id='.$id);
        } elseif (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar usuario', 'Dirección de correo electrónico no válida!'));
            redirect(base_url().'setting/edituser?id='.$id);
        } elseif (empty($role)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar usuario', 'Por favor, elija el rol del usuario!'));
            redirect(base_url().'setting/edituser?id='.$id);
        } else {
            ...
            if (count($ckEmailData) == 0) {
                ...
                if ($this->Constant_model->updateData('users', $upd_data, $id)) {
                    $this->session->set_flashdata('alert_msg', array('success', 'Actualizar usuario', 'Detalles del usuario correctamente actualizados.'));
                    redirect(base_url().'setting/edituser?id='.$id);
                }
            } else {
                $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar usuario', 'La dirección de correo electrónico ya existe en el sistema!'));
                redirect(base_url().'setting/edituser?id='.$id);
            }
        }
    }
    ...
    public function insertUser()
    {
        ...
        if (empty($fn)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo usuario', 'Por favor ingrese el nombre completo!', "$fn", "$email", "$pass", "$conpass", "$role", "$outlet"));
            redirect(base_url().'setting/adduser');
        } elseif (empty($email)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo usuario', 'Por favor, introduzca la dirección de correo electrónico!', "$fn", "$email", "$pass", "$conpass", "$role", "$outlet"));
            redirect(base_url().'setting/adduser');
        } elseif (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo usuario', 'Dirección de correo electrónico no válida!', "$fn", "$email", "$pass", "$conpass", "$role", "$outlet"));
            redirect(base_url().'setting/adduser');
        } elseif (empty($pass)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo usuario', 'Por favor, ingrese contraseña!', "$fn", "$email", "$pass", "$conpass", "$role", "$outlet"));
            redirect(base_url().'setting/adduser');
        } elseif (empty($conpass)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo usuario', 'Por favor ingrese Confirmar Contraseña!', "$fn", "$email", "$pass", "$conpass", "$role", "$outlet"));
            redirect(base_url().'setting/adduser');
        } elseif ($pass != $conpass) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo usuario', 'La contraseña debe ser la misma!', "$fn", "$email", "$pass", "$conpass", "$role", "$outlet"));
            redirect(base_url().'setting/adduser');
        } elseif (empty($role)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo usuario', 'Por favor, elija el rol del usuario!', "$fn", "$email", "$pass", "$conpass", "$role", "$outlet"));
            redirect(base_url().'setting/adduser');
        } else {
            ...
            if (count($ckEmailData) == 0) {
                ...
                if ($this->Constant_model->insertData('users', $ins_user_data)) {
                    $this->session->set_flashdata('alert_msg', array('success', 'Añadir nuevo usuario', "Se agregó exitosamente una nueva cuenta de usuario con correo : $email.", '', '', '', '', '', ''));
                    redirect(base_url().'setting/adduser');
                }
            } else {
                $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo usuario', "La Dirección de correo electrónico : $email Ya está registrado en el sistema! Por favor, intente con otra dirección de correo electrónico!", "$fn", "$email", "$pass", "$conpass", "$role", "$outlet"));
                redirect(base_url().'setting/adduser');
            }
        }
    }
    ...
    public function updateOutlet()
    {
        ...
        if (empty($outlet_name)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo Outlet', 'Por favor ingrese su nombre de Outlet!'));
            redirect(base_url().'setting/editoutlet?id='.$id);
        } elseif (empty($outlet_address)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo Outlet', 'Por favor ingrese su dirección!'));
            redirect(base_url().'setting/editoutlet?id='.$id);
        } elseif (empty($outlet_contact)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Añadir nuevo Outlet', 'Por favor ingrese su contacto de Outlet!'));
            redirect(base_url().'setting/editoutlet?id='.$id);
        } else {
            ...
            if ($this->Constant_model->updateData('outlets', $upd_outlet_data, $id)) {
                $this->session->set_flashdata('alert_msg', array('success', 'Actualización Outlet', "Outlet actualizado con éxito : $outlet_name."));
                redirect(base_url().'setting/editoutlet?id='.$id);
            }
        }
    }
    ...
    public function insertOutlet()
    {
        ...
        if (empty($outlet_name)) {
            ...
            if ($this->Constant_model->insertData('outlets', $ins_outlet_data)) {
                $this->session->set_flashdata('alert_msg', array('success', 'Añadir nuevo Outlet', "Agregado exitosamente Nuevo Outlet : $outlet_name."));
                redirect(base_url().'setting/addoutlet');
            }
        }
    }
    ...
    public function updateSiteSetting()
    {
        ...
        if (empty($site_name)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar configuración del sitio', 'Por favor ingrese el nombre de su sitio!'));
            redirect(base_url().'setting/system_setting');
        } elseif (strlen($tax) == 0) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar configuración del sitio', 'Por favor ingrese su impuesto gubernamental!'));
            redirect(base_url().'setting/system_setting');
        } elseif (!is_numeric($tax)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar configuración del sitio', 'El impuesto del gobierno debe ser numérico!'));
            redirect(base_url().'setting/system_setting');
        } else {
            ...
            if (!empty($temp_fn)) {
                ...
                if (($temp_fn_ext == 'jpg') || ($temp_fn_ext == 'png') || ($temp_fn_ext == 'jpeg')) {
                    if ($_FILES['uploadFile']['size'] > 500000) {
                        $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar configuración del sitio', 'El tamaño del archivo de carga debe ser inferior a 500 KB!'));
                        redirect(base_url().'setting/system_setting');

                        die();
                    }
                } else {
                    $this->session->set_flashdata('alert_msg', array('failure', 'Update Product', '¡Formato de archivo inválido! Cargue el formato de archivo JPG, PNG, JPEG para el logotipo de inicio de sesión!'));
                    redirect(base_url().'setting/system_setting');
            ...
            if ($this->Constant_model->updateData('site_setting', $update_data, '1')) {
                $this->session->set_flashdata('alert_msg', array('success', 'Actualizar configuración del sitio', 'Configuración del sitio exitosamente actualizada.'));
                redirect(base_url().'setting/system_setting');
            }
        }
    }
