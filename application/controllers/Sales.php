...
class Sales extends CI_Controller
{
    ...
    public function deleteSale()
    {
        ...
        if ($order_type == '1') {
            $this->session->set_flashdata('alert_msg', array('success', 'Eliminar ventas', 'Ventas eliminadas exitosamente.'));
        } elseif ($order_type == '2') {
            $this->session->set_flashdata('alert_msg', array('success', 'Eliminar retorno', 'Orden de devolución eliminada con éxito.'));
        }
    ...
    public function deleteSuspended()
    {
        ...
        if (count($ckSusData) == 1) {
            if ($this->Constant_model->deleteData('suspend', $id)) {
                ...
                $this->session->set_flashdata('alert_msg', array('success', 'Eliminar factura abierta', 'Se eliminó con éxito la factura abierta.'));
                redirect(base_url().'sales/opened_bill');
            }
        } else {
            $this->session->set_flashdata('alert_msg', array('failure', 'Eliminar factura abierta', 'Error al borrar la factura abierta! Inténtalo de nuevo!'));
            redirect(base_url().'sales/opened_bill');
        }
    }
