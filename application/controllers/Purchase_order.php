...
class Purchase_order extends CI_Controller
{
    ...
    public function deletePO()
    {
        ...
        if ($ckExistRows == 1) {
            if ($this->Constant_model->deleteData('purchase_order', $id)) {
                ...
                $this->session->set_flashdata('alert_msg', array('success', 'Eliminar orden de compra', "Orden de compra eliminada exitosamente : $po_numb"));
    ...
    public function ReceiveItemsPO()
    {
        ...
        $this->session->set_flashdata('alert_msg', array('success', 'Actualizar orden de compra', 'Artículo recibidos exitosamente.'));
    ...
    public function updatePO()
    {
        ...
        if (empty($po_numb)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar orden de compra', 'Por favor ingrese el número de orden de compra!'));
            redirect(base_url().'purchase_order/editpo?id='.$id);
        } elseif (empty($outlet)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar orden de compra', 'Por favor, seleccione para realizar la orden de Compra!'));
            redirect(base_url().'purchase_order/editpo?id='.$id);
        } elseif (empty($supplier)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar orden de compra', 'Por favor, seleccione un Proveedor para Orden de Compra!'));
            redirect(base_url().'purchase_order/editpo?id='.$id);
        } else {
            ...
            if ($ckPORows > 0) {
                $this->session->set_flashdata('alert_msg', array('failure', 'Crear orden de compra', "Número de orden de compra : $po_numb is already existing in the system! Please try another one!"));
                redirect(base_url().'purchase_order/editpo?id='.$id);
            } else {
                ...
                $this->session->set_flashdata('alert_msg', array('success', 'Actualizar orden de compra', 'Orden de compra actualizada exitosamente'));
                redirect(base_url().'purchase_order/editpo?id='.$id);
            }
        }
    }
    ...
    public function insertNewPO()
    {
        ...
        if (empty($po_numb)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Crear orden de compra', 'Por favor ingrese el número de orden de compra!'));
            redirect(base_url().'purchase_order/create_purchase_order');
        } elseif (empty($outlet)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Crear orden de compra', 'Por favor, seleccione para realizar la orden de Compra!'));
            redirect(base_url().'purchase_order/create_purchase_order');
        } elseif (empty($supplier)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Crear orden de compra', 'Por favor, seleccione un Proveedor para Orden de Compra!'));
            redirect(base_url().'purchase_order/create_purchase_order');
        } else {
            ...
            if(!empty($temp_fn)){
                if ($_FILES['uploadFile']['size'] > 2048000 ){
                    $this->session->set_flashdata('alert_msg', array('failure', 'Crear orden de compra', "Upload file size must be less than 2MB!"));
                    redirect(base_url().'purchase_order/create_purchase_order');
                    die();
                }
            }
            */
            ...
            $ckPORows = $ckPOResult->num_rows();
            if ($ckPORows > 0) {
                $this->session->set_flashdata('alert_msg', array('failure', 'Crear orden de compra', "Número de orden de compra : $po_numb ¡Ya existe en el sistema! Por favor prueba otro!"));
                redirect(base_url().'purchase_order/create_purchase_order');
            } else {
                $supplierDtaData = $this->Constant_model->getDataOneColumn('suppliers', 'id', $supplier);
                ...
                */
                ...
                $this->session->set_flashdata('alert_msg', array('success', 'Crear orden de compra', "Orden de compra exitosamente creada : $po_numb"));
                redirect(base_url().'purchase_order/po_view');
            }
            unset($ckPOResult);
            unset($ckPORows);
    ...
    public function deletePOItem()
    {
        ...
        if (count($ckFirstData) == 1) {
            if ($this->Constant_model->deleteData('purchase_order_items', $po_item_id)) {
                $this->session->set_flashdata('alert_msg', array('success', 'Actualizar orden de compra', 'Artículo de orden de compra borrado exitosamente.'));
                redirect(base_url().'purchase_order/editpo?id='.$po_id);
            }
        } else {
            $this->session->set_flashdata('alert_msg', array('failure', 'Actualizar orden de compra', 'Error al eliminar el artículo de orden de compra!'));
            redirect(base_url().'purchase_order/editpo?id='.$po_id);
        }
    }
